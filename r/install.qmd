...
aliases:
  - /Rinstall.html
  - /RInstall.html   # the 2020-2021 bookdown spelling, still in Google's index
---
...
2. Install Positron by downloading and running the macOS installer from [positron.posit.co](https://positron.posit.co/download).

**Windows:** 

1. Install R by downloading and running the .exe installer from [CRAN](https://cran.r-project.org/bin/windows/base/).
2. Install Positron by downloading and running the Windows installer from [positron.posit.co](https://positron.posit.co/download).
...
2. Install Positron [for your distribution](https://positron.posit.co/download).
...
**If you are having any difficulties with the installations or your Positron window does not look like this one, use the Request help button at the top of the page.**
...
1.  **Launch an R session within Positron**

    On Windows, click the start button and search for Positron then click on it. On Mac,
    Positron will be in your Applications folder --- double click on it. The Console at the
    bottom of the window is an R session.

2.  **Install `tidyverse`**

    In the Console pane at the bottom of the window, at the command prompt (`>`) type the following and press enter:
...
1.  **`tidyverse` is not available for R version...**
    * Solution: make sure you have the latest versions of both R ({{< var r.version >}}) and Positron ({{< var positron.version >}}).
...
4.  **rlang and/or broom still do not work**
    ...
    * `library("dplyr")`    to filter, select, arrange and summarise data
...
**If you have still not successfully installed `tidyverse` (or at least `dplyr`, `ggplot2`, `readr`, and `tidyr`) after troubleshooting, use the Request help button at the top of the page. Without these packages, you will not be able to follow along with the workshop materials.**

:::

## Packages for the intermediate workshops

The `tidyverse` is all you need for [R Introduction](intro.qmd). The other
three R workshops each use a few extra packages. Install them the same way,
in the Console at the `>` prompt --- either now, or before the workshop you
are attending:

* **R Regression Models**

    `install.packages(c("lme4", "lmerTest", "marginaleffects", "emmeans", "multilevel"))`

* **R Graphics**

    `install.packages(c("scales", "ggrepel"))`

* **R Data Wrangling** needs nothing beyond the `tidyverse`.

::: {.callout-tip}

**Success? Load one of them to check, for example `library(lme4)`. If it
returns you to the `>` prompt with no error, it is installed.**
...
be ready to start your workshop. **If you ran into any problems, use the
Request help button at the top of the page.**
