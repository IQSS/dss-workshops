...
library(emmeans)  # for contrasts among the levels of a factor
...
# | `avg_predictions()` | `marginaleffects` | predicted values, averaged over the sample |
# | `avg_slopes()`      | `marginaleffects` | marginal effects (slopes)                  |
# | `emmeans()`         | `emmeans`         | contrasts among the levels of a factor     |
#
...
#
# `emmeans` ("estimated marginal means") is built around comparing the levels of
# factors, and it comes from the world of designed experiments --- balanced
# factorial designs, where you want every pairwise contrast and a correction for
# having made many of them at once. That is what it is doing above, and it is the
# right tool for that job.
#
# For everything else in this workshop --- asking what the model predicts for
# particular values of the predictors --- we use `marginaleffects`, which is built
# around observational data and answers the question by averaging over the sample
# you actually have rather than over a balanced design you do not.
#
# ### Exercise 1
#
# **Interactions & factors**
...
# 3. Turning model coefficients into predicted probabilities
#
# ### Logistic regression
#
...
#
# For a continuous predictor we have to say which values to visit, since the model cannot try all of them:
#
  ...
  # the counterfactual grid: every person in the data, at every one of those ages
  avg_predictions(hyp_out, variables = list(age_p = age_years))

#
# `plot_predictions()` draws the same quantity:
#
...
#
# 2.  Predict the probability of working for each level of marital status. Hint: use `avg_predictions()`
