...
#
# A handy [base R cheat-sheet](base-r-cheat-sheet.pdf) is available to help you look up and remember basic syntax. In addition, a [data transformation cheat-sheet](data-transformation-cheat-sheet.pdf) provides a convenient summary of data manipulation syntax.
#
# ### Class Structure
#
...
# * We do assume you know **why** you want to learn R. If you don't, and want a comparison of R to other statistical software, see our [Data Science Tools](https://iqss.github.io/dss-workshops/general/tools.html) workshop
...
#
# This workshop will not cover how to iterate over collections of data, create your own functions, produce publication quality graphics, or fit models to data. These topics are covered in our [R Data Wrangling](https://iqss.github.io/dss-workshops/r/wrangling.html), [R Graphics](https://iqss.github.io/dss-workshops/r/graphics.html), and [R Regression Models](https://iqss.github.io/dss-workshops/r/models.html) workshops.
#
# ## R basics
#
...
#
#  3. R includes extensive documentation, including a manual named "An introduction to R". Use Positron's Help pane to locate this manual.
#
# 1. Try to get R to add 2 plus 2.
#
...
# | whitespace separated      | `read_table()`  |
...
#
# The `baby_names` object we created in the last exercise is a `data.frame`. There are many other data structures in R, but for now we'll focus on working with `data.frames`. Think of a `data.frame` as a spreadsheet. If you want to know more about R data structures, you can see a summary in our [R Data Wrangling](https://iqss.github.io/dss-workshops/r/wrangling.html#data-types-and-structures) workshop.
#
...
# ### Pipe operator
#
# There is one very special operator in R called a **pipe** operator that looks like this: `|>`. It allows us to "chain" several function calls and, as each function returns an object, feed it into the next call in a single statement, without needing extra variables to store the intermediate results. The point of the pipe is to help you write code in a way that is easier to read and understand as we will see below. The keyboard shortcut for the pipe is `Ctrl + Shift + M` (`Cmd + Shift + M` on a Mac).
#
# The pipe is built into R itself, so there is no package to load in order to use it. Let's rewrite the sequence of commands to output ordered counts for names "Alex" or "Mark".
#
...
output_of_thing_on_left |> becomes_first_argument_of_thing_on_right()

#
# Note the parentheses on the right-hand side: `|>` always pipes into a function *call*, so write `glimpse()` rather than `glimpse`.
#
...
# #### You will also meet `%>%`
#
# R gained a pipe of its own only in version 4.1, released in 2021. Before that, the same idea was provided by the `%>%` operator from the `magrittr` package, which comes with the `tidyverse`.
#
# ![](images/magrittr.png)
#
# You will meet `%>%` constantly --- in older scripts, in books, and in most of the answers you find online --- so it is worth being able to read it. For everything we do in these workshops the two behave identically:
#

#| eval: false
baby_names %>% filter(Year == 1992)   # the magrittr pipe
baby_names |> filter(Year == 1992)    # the base R pipe

#
# Read `%>%` as "then", exactly as you would read `|>`. We teach `|>` because it is part of R itself, so it works in any script whether or not you have loaded a package. The two differ in some advanced details --- `%>%` lets you write a `.` to control *where* the piped value lands, for instance --- but not in anything these workshops ask you to do.
#
# ### Exercise 2.2
#
...
#
# When you run this, R will tell you that `qplot()` was deprecated in ggplot2 3.4.0. The function still works, and we use it here because it draws a useful plot in a single line. The fuller `ggplot()` syntax --- which is what you will want for anything you intend to publish, and which gives you control over every part of a graph --- is the subject of our [R Graphics](https://iqss.github.io/dss-workshops/r/graphics.html) workshop.
#
