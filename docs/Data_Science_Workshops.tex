...
\tightlist
\item
  Use the dataset, ``talent.dta'', open a new do-file and write on the
  do-file, after the exercise save it to the folder
\item
  Examine a few selected variables using the describe, sum and codebook
  commands
\item
  Tabulate the variable, marital status (``marital''), with and without
  labels
\item
  Summarize the total household income last year (``income'') by marital
  status
\item
  Cross-tabulate marital status with respondents' type of main job
  (``job'')
\item
  Summarize the total household income last year (``income'') for
...
\item
  ...
  ``workperweek'' that will take on a value of ``1'' if a person works
  more than 40 hours per week, and ``0'' if a person works equal to or
  less than 40 hours per week
\item
  Generate a new ``marital\_dummy'' dummy variable from the original
  variable ``marital'' that will take on a value of ``1'' if a person is
  either married or partnered and ``0'' otherwise
\item
...
use gss.dta, clear drop inc

clear all keep age region happy educ sex

You can drop cases selectively using the conditional ``if'', for example

drop if sex == 1 /\emph{this will drop observtions (rows) where gender =
1}/ drop if age \textgreater{} 40 /\emph{this will drop observations
where age \textgreater{} 40}/

\subsection{Alternatively, you can keep options you
want}\label{alternatively-you-can-keep-options-you-want}

keep if sex == 0 keep if age \textless{} 40 keep if region == ``north''
\textbar{} region == ``south''
...
\item
  Rename the Sex/gender variable and give it a more intuitive name
\item
  Use codebook, describe, tab, and browse commands to know more about
  how the three variables ``A3'', ``A5'', and ``A7'' are coded and
  store, give them new names
\item
  Plot a histogram distribution for ``workperweek'' and add a normal
  curve
\item
  Give a variable label and value labels for the variable ``overwork''
\item
  Generate a new variable called ``work\_family'' and code it as 2 if a
  respondent perceived work to be more important than family, 1 if a
  respondent perceived family to be more important than work, and 0 if
  the two are of equal importance
\item
  Drop the B3C variable that is not used in our exercise
\item
  Save the changes to a new dataset called ``talent\_new.dta'' and save
  it to our folder
\end{enumerate}
