...
If we just want to inspect the coefficients, we can further pipe the summary output into the function \texttt{coef()} to obtain just the coefficients table.
...
We can see that fitted model object is a \texttt{list} structure (a type of container for different pieces of information). What have we learned by examining the fitted model object? We can see that the default output we get when printing a fitted model of class \texttt{lm} is only a very small part of the information stored within the model object. How can we access other quantities of interest from the model?
...
We can also use \texttt{function\ methods} to get more information about the fit. We've already seen the \texttt{summary()} function for \texttt{lm}, which is a good place to start:
...
We can use the \texttt{confint()} function to get interval estimates for our coefficients:
...
And we can use the \texttt{anova()} function to get an ANOVA-style table of the model:
...
Now let's refit the model with the releveled \texttt{region} variable:
...
\includegraphics{R/Rgraphics/figures/unnamed-chunk-221-1.pdf}
...
\includegraphics{R/RDataWrangling/figures/unnamed-chunk-272-1.pdf}
