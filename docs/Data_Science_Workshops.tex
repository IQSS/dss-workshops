...
Objects (data structures) can be assigned names and used in subsequent
operations:
...
\item
  ...
  by a dash) is used to save objects
\item
...
\begin{verbatim}
## # A tibble: 1 x 16
##   Name   Sex.x Count.x  Year Count_1k.x Rank.x Count_levels.x Proportion.x Sex.y Count.y Count_1k.y Rank.y Count_levels.y Proportion.y   Product  Rank
##   <chr>  <chr>   <dbl> <dbl>      <dbl>  <dbl> <chr>                 <dbl> <chr>   <dbl>      <dbl>  <dbl> <chr>                 <dbl>     <dbl> <dbl>
## 1 Taylor Boys     7688  1993       7.69     51 low                 0.00392 Girls   21266       21.3      7 medium               0.0118 163493008     1
\end{verbatim}
\end{enumerate}
...
We can list all the functions that extract particular quantities of
interest (called \texttt{extractor\ functions}) by using the
\texttt{methods()} function with the \texttt{class} argument set to the
...
We can also use \texttt{function\ methods} to get more information about
the fit:
...
It's always worth examining whether the class of model you're fitting
has a method for a particular extractor function. Here's a summary table
of some of the most often used extractor functions, which have methods
for a wide range of model classes. These are post-estimation tools you
...
\includegraphics{R/Rmodels/figures/unnamed-chunk-93-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  sat_mod <-}\StringTok{ }
\StringTok{      }\NormalTok{sat_mod }\OperatorTok{%>%}
\StringTok{      }\KeywordTok{update}\NormalTok{(}\DataTypeTok{data =} \KeywordTok{na.omit}\NormalTok{(states_data))}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  states_en_met <-}\StringTok{ }
\StringTok{      }\NormalTok{states }\OperatorTok{%>%}\StringTok{ }
...
\includegraphics{R/Rmodels/figures/unnamed-chunk-104-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  states_en_met_pop_wst <-}\StringTok{ }
\StringTok{      }\NormalTok{states }\OperatorTok{%>%}
...
\includegraphics{R/Rmodels/figures/unnamed-chunk-105-1.pdf}
...
statistical sofware use contrasts, but each software has different
...
chosen as the reference category.They're called ``treatment'' contrasts,
because of the typical use case where there is one control group (the
reference group) and one or more treatment groups that are to be
compared to the controls. It is easy to change the default contrasts to
something other than treatment contrasts, though this is rarely needed.
More often, we may want to change the reference group in treatment
contrasts or get all sets of pairwise contrasts between factor levels.
...
\NormalTok{  mod_region <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(csat }\OperatorTok{~}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{region, }\DataTypeTok{data =}\NormalTok{ states_data)}
  \KeywordTok{summary}\NormalTok{(mod_region) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{coef}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  mod_region }\OperatorTok{%>%}
\StringTok{      }\KeywordTok{emmeans}\NormalTok{(}\DataTypeTok{specs =}\NormalTok{ pairwise }\OperatorTok{~}\StringTok{ }\NormalTok{region)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  hyp_out }\OperatorTok{%>%}\StringTok{ }
\StringTok{      }\KeywordTok{allEffects}\NormalTok{() }\OperatorTok{%>%}
...
\includegraphics{R/Rmodels/figures/unnamed-chunk-119-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  eff_df <-}\StringTok{ }
\StringTok{      }\NormalTok{hyp_out }\OperatorTok{%>%}\StringTok{ }
\StringTok{      }\KeywordTok{allEffects}\NormalTok{(}\DataTypeTok{xlevels =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{age_p =}\NormalTok{ age_years)) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# override defaults for age}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  NH11 <-}\StringTok{ }
\StringTok{      }\NormalTok{NH11 }\OperatorTok{%>%}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  mod_wk_age_mar }\OperatorTok{%>%}\StringTok{ }
\StringTok{      }\KeywordTok{allEffects}\NormalTok{() }\OperatorTok{%>%}
...
where \(U_{0j}\) is the random intercept for the \(j\)th
\texttt{school}. Let's implement this in R using \texttt{lmer()}:
...
\texttt{lmer()} models using the \texttt{anova()} function. With mixed
effects models, this will produce a likelihood ratio test.
...
R IDE (such as Rstudio) to see a list of functions starting with
\texttt{geom\_}.
...
\NormalTok{hp2001Q1 <-}\StringTok{ }\NormalTok{housing }\OperatorTok{%>%}\StringTok{ }\KeywordTok{filter}\NormalTok{(Date }\OperatorTok{==}\StringTok{ }\FloatTok{2001.25}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
But what if we want to include points and labels? We can use
\texttt{geom\_text\_repel()} to keep labels from overlapping the points
and each other.
...
\tightlist
\item
  Variables are \textbf{mapped} to aesthetics inside the \texttt{aes()}
  function.
\end{enumerate}
...
\tightlist
\item
  Constants are \textbf{assigned} to aesthetics outside the
  \texttt{aes()} call
\end{enumerate}
...
order to change them you have to first determine which stat the geom
...
We can change the binning scheme by passing the \texttt{binwidth}
argument to the \texttt{stat\_bin()} function:
...
\includegraphics{R/Rgraphics/figures/unnamed-chunk-168-1.pdf}
...
\NormalTok{## Error: stat_count() must not be used with a y aesthetic.  }
\end{Highlighting}
\end{Shaded}
...
function. The \texttt{identity} function returns the same output as the
input.
...
\tightlist
\item
  BONUS 1: Allow the smoothing line created in the last plot to vary
  across the levels of \texttt{Region}. Hint: map \texttt{Region} to the
...
\tightlist
\item
  BONUS 2: Overlay a loess \texttt{(method\ =\ "loess")} smoothing line
  on top of the scatter plot using \texttt{geom\_line()}. Hint: change
...
\includegraphics{R/Rgraphics/figures/unnamed-chunk-197-1.pdf} You can
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{home_land_byyear <-}\StringTok{ }
\StringTok{    }\NormalTok{housing_byyear }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{pivot_longer}\NormalTok{(}\DataTypeTok{cols =} \KeywordTok{c}\NormalTok{(Home_Value_Mean, Land_Value_Mean),}
...
\includegraphics{R/RDataWrangling/figures/unnamed-chunk-262-1.pdf}
...
In Python we can assign an object (data structure) to an name using the
\texttt{=} operator.
...
\tightlist
\item
  Make sure you have both the text and the list of characters. Open and
  read both ``Alice\_in\_wonderland.txt'' and ``Characters.txt'' if you
...
\tightlist
\item
  Which chapter has the most words? Split the text into chapters (i.e.,
  split on ``CHAPTER'') and use a for-loop to iterate over the chapters.
...
\tightlist
\item
  How many times is each character mentioned in the text? Iterate over
  the list of characters using a for-loop. For each character, call the
  count method with that character as the argument.
\end{enumerate}
...
## 0                       Purchase         1991.0              ...                Good. Object is well descr...   
## 1                           Gift         1898.0              ...                Good. Object is well descr...   
## 2                       Purchase         2007.0              ...                Good. Object is well descr...   
## 3                        Bequest         1943.0              ...                Good. Object is well descr...   
## 4  Partial Gift/Partial Purchase         1991.0              ...                Good. Object is well descr...   
## 5                           Gift         1996.0              ...                Adequate. Object is adequa...   
## 6                           Gift         1929.0              ...                Good. Object is well descr...   
## 7                           Gift         2001.0              ...                Good. Object is well descr...   
## 8                           Gift         1978.0              ...                Best. Object is extensivel...   
## 9                           None            NaN              ...                Good. Object is well descr...   
## 
##                        worktypes  
## 0  [{'worktypeid': '342', 'wo...  
## 1  [{'worktypeid': '125', 'wo...  
...
## 8                            Gift         1978.0              ...                Best. Object is extensivel...   
## 9                            None            NaN              ...                Good. Object is well descr...   
## 10                       Purchase         2000.0              ...                Best. Object is extensivel...   
## 11                       Transfer         2011.0              ...                Good. Object is well descr...   
## 12                           Gift         2003.0              ...                Good. Object is well descr...   
## 13                       Purchase         1953.0              ...                Good. Object is well descr...   
## 14                       Purchase         2007.0              ...                Good. Object is well descr...   
## 15                       Purchase         2010.0              ...                Good. Object is well descr...   
## 16                       Purchase         2007.0              ...                Good. Object is well descr...   
## 17                       Purchase         1973.0              ...                Good. Object is well descr...   
## 18                           None            NaN              ...                Good. Object is well descr...   
## 19                       Purchase         2010.0              ...                Adequate. Object is adequa...   
## 20                       Purchase         1997.0              ...                Good. Object is well descr...   
## 21                           None            NaN              ...                Good. Object is well descr...   
## 22  Long Term Loan and Promise...         2006.0              ...                Good. Object is well descr...   
## 23                           Gift         1995.0              ...                Best. Object is extensivel...   
## 24                           Gift         2004.0              ...                Good. Object is well descr...   
## 25                           Gift         2002.0              ...                Good. Object is well descr...   
## 26                        Bequest         1985.0              ...                Adequate. Object is adequa...   
## 27                       Purchase         1982.0              ...                Adequate. Object is adequa...   
## 28                           Gift         1996.0              ...                Adequate. Object is adequa...   
## 29                           Gift         1984.0              ...                Unchecked. Object informat...   
## 30                       Purchase         2010.0              ...                Good. Object is well descr...   
## 31                       Purchase         2008.0              ...                Good. Object is well descr...   
## 32                           None            NaN              ...                Good. Object is well descr...   
## 33                       Purchase         2008.0              ...                Good. Object is well descr...   
## 34                       Purchase         1995.0              ...                Good. Object is well descr...   
## 35                       Purchase         1979.0              ...                Adequate. Object is adequa...   
## 36                       Purchase         1994.0              ...                Good. Object is well descr...   
## 37                        Bequest         2007.0              ...                Best. Object is extensivel...   
## 38                       Purchase         2008.0              ...                Good. Object is well descr...   
## 39                        Bequest         1972.0              ...                Good. Object is well descr...   
## 40                       Purchase         1967.0              ...                Unchecked. Object informat...   
## 41                           None            NaN              ...                Good. Object is well descr...   
## 42                           Gift         1972.0              ...                Good. Object is well descr...   
## 43                       Purchase         2002.0              ...                Good. Object is well descr...   
## 44                           Gift         1993.0              ...                Good. Object is well descr...   
## 45                        Bequest         1943.0              ...                Good. Object is well descr...   
## 46                           Gift         1953.0              ...                Good. Object is well descr...   
## 47                       Transfer         2011.0              ...                Good. Object is well descr...   
## 48                       Purchase         2009.0              ...                Good. Object is well descr...   
## 49                       Purchase         2006.0              ...                Best. Object is extensivel...   
## 
##                         worktypes  
## 0   [{'worktypeid': '342', 'wo...  
## 1   [{'worktypeid': '125', 'wo...  
...
## 8   [{'worktypeid': '242', 'wo...  
## 9   [{'worktypeid': '278', 'wo...  
## 10  [{'worktypeid': '346', 'wo...  
## 11  [{'worktypeid': '259', 'wo...  
## 12  [{'worktypeid': '259', 'wo...  
## 13  [{'worktypeid': '317', 'wo...  
## 14  [{'worktypeid': '259', 'wo...  
## 15  [{'worktypeid': '259', 'wo...  
## 16  [{'worktypeid': '278', 'wo...  
## 17  [{'worktypeid': '259', 'wo...  
## 18  [{'worktypeid': '278', 'wo...  
## 19  [{'worktypeid': '50', 'wor...  
## 20  [{'worktypeid': '259', 'wo...  
## 21  [{'worktypeid': '278', 'wo...  
## 22  [{'worktypeid': '317', 'wo...  
## 23  [{'worktypeid': '346', 'wo...  
## 24  [{'worktypeid': '125', 'wo...  
## 25  [{'worktypeid': '7', 'work...  
## 26  [{'worktypeid': '2', 'work...  
## 27  [{'worktypeid': '317', 'wo...  
## 28  [{'worktypeid': '50', 'wor...  
## 29  [{'worktypeid': '208', 'wo...  
## 30  [{'worktypeid': '259', 'wo...  
## 31  [{'worktypeid': '389', 'wo...  
## 32  [{'worktypeid': '259', 'wo...  
## 33  [{'worktypeid': '317', 'wo...  
## 34  [{'worktypeid': '210', 'wo...  
## 35  [{'worktypeid': '317', 'wo...  
## 36  [{'worktypeid': '259', 'wo...  
## 37  [{'worktypeid': '242', 'wo...  
## 38  [{'worktypeid': '259', 'wo...  
## 39  [{'worktypeid': '100', 'wo...  
## 40  [{'worktypeid': '242', 'wo...  
## 41  [{'worktypeid': '100', 'wo...  
## 42  [{'worktypeid': '242', 'wo...  
## 43  [{'worktypeid': '389', 'wo...  
## 44  [{'worktypeid': '278', 'wo...  
## 45  [{'worktypeid': '317', 'wo...  
## 46  [{'worktypeid': '242', 'wo...  
## 47  [{'worktypeid': '259', 'wo...  
## 48  [{'worktypeid': '259', 'wo...  
...
##  'figcaption': 'Covered tripod vessel, Chinese, Han dynasty, 2nd–1st century '
##                'BCE. Earthenware with cold painted pigments. Harvard Art '
##                'Museums/Arthur M. Sackler Museum, Partial gift of the Walter '
##                'C. Sedgwick Foundation and partial purchase through the Ernest '
##                'B. and Helen Pratt Dane Fund for the Acquisition of Oriental '
##                'Art, 2006.170.171.',
##  'localtion1': '32 Quincy Street',
##  'location2': 'Cambridge',
##  'time': '12:30pm - 1:00pm',
##  'title': 'Gallery Talk: A Closer Look at Funerary Ceramics from the Han '
##           'Period'}
\end{verbatim}
...
## 0                                    Tuesday, March 10, 2020    ...        32 Quincy Street  Cambridge
## 1                                  Wednesday, March 11, 2020    ...        32 Quincy Street  Cambridge
## 2        Photo by Stefan Ruiz for  Wednesday, March 11, 2020    ...        32 Quincy Street  Cambridge
## 3   1958 D. A. Flentrop organ,...   Thursday, March 12, 2020    ...      29 Kirkland Street  Cambridge
## 4   Dante Gabriel Rossetti, Br...   Thursday, March 12, 2020    ...        32 Quincy Street  Cambridge
## 5                                     Friday, March 13, 2020    ...        32 Quincy Street  Cambridge
## 6            Photo: Tim Correira.  Wednesday, March 18, 2020    ...        32 Quincy Street  Cambridge
## 7            Group of Polygnotos,   Thursday, March 19, 2020    ...        32 Quincy Street  Cambridge
## 8                  Tawaraya Sōri,   Thursday, March 19, 2020    ...        32 Quincy Street  Cambridge
## 9                  Maruyama Ōkyo,    Tuesday, March 24, 2020    ...        32 Quincy Street  Cambridge
## 10            László Moholy-Nagy,  Wednesday, March 25, 2020    ...        32 Quincy Street  Cambridge
## 11  1958 D. A. Flentrop organ,...   Thursday, March 26, 2020    ...      29 Kirkland Street  Cambridge
## 12       Mark Bradford, American,   Thursday, March 26, 2020    ...        32 Quincy Street  Cambridge
## 13  Manufactured by Wedgwood, ...     Friday, March 27, 2020    ...        32 Quincy Street  Cambridge
## 14                                  Saturday, March 28, 2020    ...        32 Quincy Street  Cambridge
## 15  Close-up of a glass muller...   Saturday, March 28, 2020    ...        32 Quincy Street  Cambridge
## 16                                  Saturday, March 28, 2020    ...        32 Quincy Street  Cambridge
## 17                                    Sunday, March 29, 2020    ...        32 Quincy Street  Cambridge
## 18                                    Sunday, March 29, 2020    ...        32 Quincy Street  Cambridge
## 19                 Maruyama Ōkyo,    Tuesday, March 31, 2020    ...        32 Quincy Street  Cambridge
## 20  Professor Kurt Wehlte in 1...   Wednesday, April 1, 2020    ...        32 Quincy Street  Cambridge
## 21  1958 D. A. Flentrop organ,...    Thursday, April 2, 2020    ...      29 Kirkland Street  Cambridge
## 22  Modern forgery of an ancie...      Friday, April 3, 2020    ...        32 Quincy Street  Cambridge
## 23                 Maruyama Ōkyo,    Saturday, April 4, 2020    ...        32 Quincy Street  Cambridge
## 24                                   Saturday, April 4, 2020    ...        32 Quincy Street  Cambridge
## 25                                   Saturday, April 4, 2020    ...        32 Quincy Street  Cambridge
## 26                                     Sunday, April 5, 2020    ...        32 Quincy Street  Cambridge
## 27                                     Sunday, April 5, 2020    ...        32 Quincy Street  Cambridge
## 28  1958 D. A. Flentrop organ,...    Thursday, April 9, 2020    ...      29 Kirkland Street  Cambridge
## 29  Courtesy of Blue Flower Ar...    Thursday, April 9, 2020    ...        32 Quincy Street  Cambridge
## ..                            ...                        ...    ...                     ...        ...
## 38          Claude Monet, French,     Friday, April 17, 2020    ...        32 Quincy Street  Cambridge
## 39                                  Saturday, April 18, 2020    ...        32 Quincy Street  Cambridge
## 40                                  Saturday, April 18, 2020    ...        32 Quincy Street  Cambridge
## 41                                    Sunday, April 19, 2020    ...        32 Quincy Street  Cambridge
## 42                                    Sunday, April 19, 2020    ...        32 Quincy Street  Cambridge
## 43                 Maruyama Ōkyo,    Tuesday, April 21, 2020    ...        32 Quincy Street  Cambridge
## 44  Close-up of a glass muller...    Tuesday, April 21, 2020    ...        32 Quincy Street  Cambridge
## 45  1958 D. A. Flentrop organ,...   Thursday, April 23, 2020    ...      29 Kirkland Street  Cambridge
## 46            László Moholy-Nagy,   Thursday, April 23, 2020    ...        32 Quincy Street  Cambridge
## 47                                  Saturday, April 25, 2020    ...        32 Quincy Street  Cambridge
## 48                                  Saturday, April 25, 2020    ...        32 Quincy Street  Cambridge
## 49                                    Sunday, April 26, 2020    ...        32 Quincy Street  Cambridge
## 50                                    Sunday, April 26, 2020    ...        32 Quincy Street  Cambridge
## 51                 Maruyama Ōkyo,    Tuesday, April 28, 2020    ...        32 Quincy Street  Cambridge
## 52          Lee Lozano, American,   Thursday, April 30, 2020    ...        32 Quincy Street  Cambridge
## 53                                       Friday, May 1, 2020    ...        32 Quincy Street  Cambridge
## 54                 Maruyama Ōkyo,     Wednesday, May 6, 2020    ...        32 Quincy Street  Cambridge
## 55                 Maruyama Ōkyo,      Saturday, May 9, 2020    ...        32 Quincy Street  Cambridge
## 56   Charles-Marie Dulac, French,    Wednesday, May 13, 2020    ...        32 Quincy Street  Cambridge
## 57      Dorothea Lange, American,      Tuesday, May 19, 2020    ...        32 Quincy Street  Cambridge
## 58  Close-up of a glass muller...      Tuesday, May 19, 2020    ...        32 Quincy Street  Cambridge
## 59           Photo: Tim Correira.    Wednesday, May 20, 2020    ...        32 Quincy Street  Cambridge
## 60             Jost Amman, Swiss,     Thursday, May 21, 2020    ...        32 Quincy Street  Cambridge
## 61                                      Monday, May 25, 2020    ...        32 Quincy Street  Cambridge
## 62                 Maruyama Ōkyo,      Tuesday, May 26, 2020    ...        32 Quincy Street  Cambridge
## 63                  © Susan Young    Wednesday, May 27, 2020    ...        32 Quincy Street  Cambridge
## 64  Lewis W. Rubenstein, Ameri...    Wednesday, May 27, 2020    ...        32 Quincy Street  Cambridge
## 65                                    Thursday, May 28, 2020    ...        32 Quincy Street  Cambridge
## 66            László Moholy-Nagy,     Thursday, May 28, 2020    ...        32 Quincy Street  Cambridge
## 67           Photo: Tim Correira.   Wednesday, June 17, 2020    ...        32 Quincy Street  Cambridge
## 
## [68 rows x 6 columns]
\end{verbatim}
...
\item
  ...
  \texttt{A3}, \texttt{A5}, and \texttt{A7} are coded and stored, give
  them new names
\end{enumerate}
