...
A large number of contributed packages are available (\textgreater{}
15,000). If you are looking for a package for a specific task,
...
\item
  ...
  first time you use the package.
\item
  Load the package into your R session's search path using the
  \texttt{library()} function. This needs to be done each time you use
  the package.
\end{enumerate}
...
\tightlist
\item
  Map the size of the points to \texttt{HDI\_Rank}
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ggsave}\NormalTok{(p5, }\DataTypeTok{file =} \StringTok{"myplot.pdf"}\NormalTok{, }\DataTypeTok{device =} \StringTok{"pdf"}\NormalTok{, }\DataTypeTok{height =} \DecValTok{6}\NormalTok{, }\DataTypeTok{width =} \DecValTok{6}\NormalTok{, }\DataTypeTok{units =} \StringTok{"in"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  so that the axis text and facet label background are blue. Hint:
  \texttt{axis\_text} and \texttt{strip\_background}.
\end{enumerate}
...
\StringTok{    }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{axis_title =} \KeywordTok{element_text}\NormalTok{(}\DataTypeTok{color =} \StringTok{"blue"}\NormalTok{),}
          \DataTypeTok{strip_background =} \KeywordTok{element_text}\NormalTok{(}\DataTypeTok{fill =} \StringTok{"blue"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# map(thing to iterate over, task to do in each iteration)}
\end{Highlighting}
\end{Shaded}
...
The \texttt{stringr} package within \texttt{tidyverse} provides
functions to \emph{detect}, \emph{locate}, \emph{extract}, \emph{match},
\emph{replace}, \emph{combine} and \emph{split} strings (among other
things).
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#str_subset(excel_sheets(boy_file_names[1]), pattern = "Table 1")}
...
to get a list or vector of inputs and obtain the return values. This is
very convenient when a function already exists that does exactly what
...
\NormalTok{   ## body of function - where stuff happens ##}
\NormalTok{   new_object <-}\StringTok{ }\NormalTok{arg1 }\OperatorTok{*}\StringTok{ }\NormalTok{arg2}
\NormalTok{   output1 <-}\StringTok{ }\KeywordTok{thing_to_do}\NormalTok{(arg1)}
\NormalTok{   output2 <-}\StringTok{ }\KeywordTok{another_thing_to_do}\NormalTok{(output1, new_object)}
   \KeywordTok{return}\NormalTok{(output2)}
\NormalTok{\}}
...
\KeywordTok{get_data_sheet_name}\NormalTok{(anyfile)}
\KeywordTok{get_data_sheet_name}\NormalTok{(anyfile, }\DataTypeTok{term=}\StringTok{"Table 2"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{tmp <-}\StringTok{ }\KeywordTok{read_excel}\NormalTok{(}
  ...
  \DataTypeTok{sheet =} \KeywordTok{get_data_sheet_name}\NormalTok{(boy_file_names[}\DecValTok{1}\NormalTok{],}
                              \DataTypeTok{term =} \StringTok{"Table 1"}\NormalTok{),}
...
elements in \texttt{boysNames}, a task I leave to you.
...
\item
  ...
  returns a modified version including only columns named
  \texttt{Name...2}, \texttt{Name...6}, \texttt{Count...3}, and
  \texttt{Count...7}.
\item
...
\begin{Highlighting}[]
\NormalTok{boysNames[[}\DecValTok{1}\NormalTok{]]}
\KeywordTok{bind_rows}\NormalTok{(}\KeywordTok{select}\NormalTok{(boysNames[[}\DecValTok{1}\NormalTok{]], Name...}\DecValTok{2}\NormalTok{, Count...}\DecValTok{3}\NormalTok{),}
          \KeywordTok{select}\NormalTok{(boysNames[[}\DecValTok{1}\NormalTok{]], }\DataTypeTok{Name...2 =}\NormalTok{ Name...}\DecValTok{6}\NormalTok{, }\DataTypeTok{Count...3 =}\NormalTok{ Count...}\DecValTok{7}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{glimpse}\NormalTok{(boysNames) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{head}\NormalTok{()}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{year_column <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(data, name) \{}
  \KeywordTok{mutate}\NormalTok{(data, }\DataTypeTok{year =} \KeywordTok{as.integer}\NormalTok{(name))}
\NormalTok{\}}

\NormalTok{boysNames <-}\StringTok{ }\KeywordTok{imap}\NormalTok{(boysNames, year_column)}
...
\NormalTok{read_baby_names <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(file) \{}
  \KeywordTok{read_excel}\NormalTok{(}
    \DataTypeTok{path =}\NormalTok{ file,}
    \DataTypeTok{sheet =} \KeywordTok{get_data_sheet_name}\NormalTok{(file, }
                                \DataTypeTok{term =} \StringTok{"Table 1"}\NormalTok{),}
    \DataTypeTok{skip =} \DecValTok{6}
\NormalTok{  )}
\NormalTok{\}}
...
\KeywordTok{glimpse}\NormalTok{(}\KeywordTok{read_baby_names}\NormalTok{(boy_file_names[}\DecValTok{1}\NormalTok{]))}

\NormalTok{  ## 3. Use the `map` function to read data from all the Excel files,}
\NormalTok{  ##    using the function you wrote in step 1.}

\NormalTok{boysNames <-}\StringTok{ }\KeywordTok{map}\NormalTok{(boy_file_names, read_baby_names)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  ##    returns a modified version including only columns named `Name...2`,}
\NormalTok{  ##    `Name...6`, `Count...3`, or `Count...7`.}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## 1.  write a function that does all the cleanup}
\NormalTok{cleanupNamesData <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(file)\{}
\NormalTok{  selected <-}\StringTok{ }\NormalTok{file }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{drop_na}\NormalTok{(Name...}\DecValTok{2}\NormalTok{) }\OperatorTok{%>%}
\StringTok{    }\KeywordTok{select}\NormalTok{(}\KeywordTok{matches}\NormalTok{(}\StringTok{"Name|Count"}\NormalTok{))}
   
  \KeywordTok{bind_rows}\NormalTok{(}\KeywordTok{select}\NormalTok{(selected, }\DataTypeTok{Name =}\NormalTok{ Name...}\DecValTok{2}\NormalTok{, }\DataTypeTok{Count =}\NormalTok{ Count...}\DecValTok{3}\NormalTok{),}
            \KeywordTok{select}\NormalTok{(selected, }\DataTypeTok{Name =} \KeywordTok{matches}\NormalTok{(}\StringTok{"Name...6|Name...7|Name...8"}\NormalTok{),}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## 1.  Turn the list of boys names data.frames into a single table.}

\NormalTok{boysNames <-}\StringTok{ }\KeywordTok{bind_rows}\NormalTok{(boysNames)}

\NormalTok{## 2.  Create a directory under `data/all` and write the data to a `.csv`}
\NormalTok{file.}
...
\NormalTok{## 3.  Finally, repeat the previous exercise, this time working with the data}
\ControlFlowTok{in}\NormalTok{ one big table.}
\NormalTok{## What were the five most popular names in 2013?}
