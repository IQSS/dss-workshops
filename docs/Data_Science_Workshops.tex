...
\item
  Basic data manipulation used to clean datasets
\item
...
\subsubsection{Text editors \& Integrated Development Environments
(IDEs)}\label{text-editors-integrated-development-environments-ides}

There are many different text editors and IDEs available for R. Here are
a few popular ones:
...
is a good R-specific IDE with many features.

\subsubsection{Literate programming versus source
code}\label{literate-programming-versus-source-code}
...
\item
  ...
  in a text file with a \texttt{.R} or \texttt{.r} suffix. Writing
...
\item
  ...
  \emph{Markdown} and which has a \texttt{.Rmd} suffix. \emph{Markdown}
  is easy to write and designed to be human-readable. Markdown is the
  format of choice if you intend to run your code interactively, by
  running small pieces of code and looking at the results. Many
  researchers use Markdown to write their journal papers, dissertations,
  and statistics/math class notes, since it is easy to use and to
  convert into other formats later, such as HTML (for a webpage), MS
  Word, or PDF (via LaTeX).
\end{enumerate}

Here are some resources for learning more about \texttt{Rmarkdown} and
RStudio:
...
\textbf{GOAL: To learn how to work with grouped data.} In particular:
...
\textbf{GOAL: To learn how to aggregate data to create summaries.} In
particular:
...
\tightlist
\item
  Using the \texttt{summarize()} function to collapse data into
  summaries
\item
  Combining the \texttt{summarize()} and \texttt{group\_by()} functions
  to create summaries within groups
\end{enumerate}
...
Notice that \textbf{the body of the for-loop is indented}. This is
important, because it is this indentation that defines the \emph{body}
of the loop --- the place where things are done. \textbf{White space
matters in Python!} A simple example:
