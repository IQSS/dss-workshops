...
The following \texttt{tidyverse} and \texttt{rmarkdown} cheatsheets will
...
The following \texttt{tidyverse}, `\texttt{ggplot2}, and
...
There are several things that make our goal challenging. Lets take a
look at the data:
...
them. Fortunately, writing functions in R is easy. Functions require 3
elements:
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# map(thing to iterate over, }
\CommentTok{#     function to do things, }
...
Finally, we will want to do this for all the elements in
\texttt{boysNames}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{boysNames <-}\StringTok{ }\KeywordTok{map}\NormalTok{(boysNames, drop_na)}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  look at the \texttt{?matches} function.
\end{enumerate}
...
\NormalTok{  ##   HINT: look at the `?matches` function.}
...
\NormalTok{## to match each of these different column names. HINT: see `matches`.}
