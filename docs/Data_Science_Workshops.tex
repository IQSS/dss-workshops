...
We can use the \texttt{imap()} function --- which stands for `index' mapping --- to do this operation. We know that the \texttt{mutate()} function can create a new column for us within a data frame. Here, we create a column called \texttt{Year} within each data frame (indexed by \texttt{.x}) that takes the name of each list element (indexed by \texttt{.y}), converts it from character (e.g., \texttt{"1996"}) to integer (e.g., \texttt{1996}), and repeats it along the rows of the data frame.
