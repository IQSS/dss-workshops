...
\textbf{choices}, such as \texttt{if} and \texttt{else} statements,
which allow you to run different code depending on the input
...
\NormalTok{x }\OperatorTok{>} \DecValTok{7} \OperatorTok{|}\NormalTok{ x }\OperatorTok{<} \DecValTok{3} \CommentTok{# two conditions combined}
\end{Highlighting}
\end{Shaded}
...
We can also count the number of times the ``Bunny'' OR the ``Duck''
character appear in a given chapter:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bunny_OR_duck_count }\OperatorTok{=}\NormalTok{ alice_chapters.count(}\StringTok{"Bunny"} \KeywordTok{or} \StringTok{"Duck"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Or, the number of times the ``Alice'' AND the ``Eaglet'' character
appear in a given chapter:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{alice_AND_eaglet_count }\OperatorTok{=}\NormalTok{ alice_chapters.count(}\StringTok{"Alice"} \KeywordTok{and} \StringTok{"Eaglet"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
