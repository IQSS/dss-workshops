...
\author{}
\date{\vspace{-2.5em}February 2021}
...
\item
  In RStudio go to \texttt{File\ -\textgreater{}\ New\ Project}. Choose \texttt{Existing\ Directory} and browse to the workshop materials directory on your desktop. This will create an \texttt{.Rproj} file for your project and will automaticly change your working directory to the workshop materials directory.
\item
...
\item
  Functions must be written with parentheses, even if there is nothing within them; for example: \texttt{ls()}
\end{itemize}

\hypertarget{function-calls}{%
\subsection{Function calls}\label{function-calls}}

\textbf{Functions perform actions} --- they take some input, called \texttt{arguments} and return some output (i.e., a result). Here's a schematic of how a function works:
...
\end{Shaded}
  ...
  The \texttt{help} function can be used to look up the documentation for a function, or to look up the documentation to a package. We can learn how to use the \texttt{stats} package by reading its documentation like this:
...
\end{Shaded}
\item
  If you know the name of the package you want to use, then Googling ``R \emph{package-name}'' will often get you to the documentation. Packages are hosted on several different repositories, including:
  ...
  \end{itemize}
\item
  If you know the type of analysis you want to perform, you can Google ``CRAN Task Views'', where there are curated lists of packages \url{https://cran.r-project.org/web/views/}. If you want to know which packages are popular, you can look at \url{https://r-pkg.org}.
\end{enumerate}
...
To read data from a file, you have to know what kind of file it is. The table below lists functions from the \texttt{readr} package, which is part of \texttt{tidyverse}, that can import data from common plain-text formats.
...
\textbf{Note} You may notice that there exist similar functions in one of the built-in packages in R called \texttt{utils}, e.g., \texttt{read.csv} and \texttt{read.delim}. These are legacy functions that tend to be slower and less robust than the \texttt{readr} functions. One way to tell them apart is that the faster more robust versions use underscores in their names (e.g., \texttt{read\_csv}) while the older functions use dots (e.g., \texttt{read.csv}). Our advice is to use the more robust newer versions, i.e., the ones with underscores.

\hypertarget{baby-names-data}{%
\subsection{Baby names data}\label{baby-names-data}}

As an example project we will analyze the popularity of baby names in the US from 1960 through 2017. The data were retrieved from \url{https://catalog.data.gov/dataset/baby-names-from-social-security-card-applications-national-level-data}.
...
\end{Shaded}
\item
  BONUS (optional): Save the \texttt{baby\_names} data as an R data set \texttt{babynames.rds}.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Filter data, keeping "Alex" and "Mark" in year 1992, and}
\CommentTok{\# assign to a new object "baby\_names\_alexmark" }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Filter data keeping rows for name "Diana" and }
\CommentTok{\# assign to a new object called "baby\_names\_diana"}
...
\begin{Highlighting}[]
\CommentTok{\# Use qplot() function to plot Counts (y) by Year (x). }
\CommentTok{\# Split trends by Sex using color.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Use piping to add a new column to the data, called Count\_1k, }
\CommentTok{\# which rescales counts to thousands}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Group baby\_names by Year and Sex and rank Count\_1k in descending order,}
\CommentTok{\# within each group (calling the resulting new column "Rank"). }
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Rank =} \KeywordTok{rank}\NormalTok{(}\KeywordTok{desc}\NormalTok{(Count\_1k))) }\OperatorTok{\%\textgreater{}\%}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Use case\_when() to recode the newly created Rank column into:}
\CommentTok{\# low (\textless{}=10), high (\textgreater{}40), and medium (all others).}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Use \texttt{mutate()} and \texttt{group\_by()} to create a column named \texttt{Proportion} where \texttt{Proportion\ =\ Count/sum(Count)} for each \texttt{Year\ X\ Sex} group. Use pipes wherever it makes sense.
...
\end{Shaded}
\item
  Filter the baby names data to display only the most popular name for each \texttt{Year\ X\ Sex} group. Keep only the columns: \texttt{Year}, \texttt{Name}, \texttt{Sex}, and \texttt{Proportion}.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Use summarize() to output the total number of boy\textquotesingle{}s and girl\textquotesingle{}s names in the sample}
\NormalTok{baby\_names }\OperatorTok{\%\textgreater{}\%}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Group baby\_names by Year and calculate the sum of Count }
\CommentTok{\# Call the resulting column "Total" }
\CommentTok{\# Assign the result to an object called "bn\_by\_year" and remember to ungroup!}
...
\CommentTok{\# Use save() function from the \textasciigrave{}base\textasciigrave{} R package to record some objects}
\CommentTok{\# into a file named "myDataFiles.RData"}
...
\NormalTok{  states\_ex\_sat \textless{}{-}}\StringTok{ }\NormalTok{states }\OperatorTok{\%\textgreater{}\%}\StringTok{ }
\StringTok{      }\KeywordTok{select}\NormalTok{(expense, csat)}
  
  \KeywordTok{summary}\NormalTok{(states\_ex\_sat)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
  ...
  \KeywordTok{cor}\NormalTok{(states\_ex\_sat, }\DataTypeTok{use =} \StringTok{"pairwise"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
  ...
  \KeywordTok{qplot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ expense, }\DataTypeTok{y =}\NormalTok{ csat, }\DataTypeTok{geom =} \StringTok{"point"}\NormalTok{, }\DataTypeTok{data =}\NormalTok{ states\_ex\_sat)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  sat\_mod \textless{}{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(csat }\OperatorTok{\textasciitilde{}}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{expense, }\CommentTok{\# regression formula}
                \DataTypeTok{data =}\NormalTok{ states) }\CommentTok{\# data }
...
\begin{Shaded}
\begin{Highlighting}[]
  \KeywordTok{lm}\NormalTok{(csat }\OperatorTok{\textasciitilde{}}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{expense }\OperatorTok{+}\StringTok{ }\NormalTok{percent, }\DataTypeTok{data =}\NormalTok{ states) }\OperatorTok{\%\textgreater{}\%}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
  \CommentTok{\# split plotting area into 2 x 2 grid}
  \KeywordTok{par}\NormalTok{(}\DataTypeTok{mfrow =} \KeywordTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{))}

  \CommentTok{\# plot model diagnostics}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# drop missing values for the 4 variables of interest}
\NormalTok{states\_voting\_cleaned \textless{}{-}}\StringTok{ }\NormalTok{states }\OperatorTok{\%\textgreater{}\%}
\StringTok{    }\KeywordTok{select}\NormalTok{(csat, expense, house, senate) }\OperatorTok{\%\textgreater{}\%}
\StringTok{    }\KeywordTok{drop\_na}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

The \texttt{drop\_na()} function omits rows with missing values --- that is, it performs \emph{listwise deletion} of observations. In some situations, we may need more flexibility. If, for example, we only want to exclude rows that have missing data for a subset of variables, we could do the following:

\begin{Shaded}
\begin{Highlighting}[]
  \CommentTok{\# pseudocode}
\NormalTok{  df \textless{}{-}}\StringTok{ }\NormalTok{df }\OperatorTok{\%\textgreater{}\%}
\StringTok{    }\KeywordTok{filter}\NormalTok{(}\OperatorTok{!}\KeywordTok{na.omit}\NormalTok{(var\_name))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  sat\_voting\_mod \textless{}{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(csat }\OperatorTok{\textasciitilde{}}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{expense }\OperatorTok{+}\StringTok{ }\NormalTok{house }\OperatorTok{+}\StringTok{ }\NormalTok{senate,}
                       \DataTypeTok{data =}\NormalTok{ states\_voting\_cleaned)}
...
To compare models, \emph{we must fit them to the same data}. This is why we needed to clean the dataset. Now let's update our first model to use these cleaned data:

\begin{Shaded}
\begin{Highlighting}[]
  \CommentTok{\# update model using the cleaned data}
\NormalTok{  sat\_mod \textless{}{-}}\StringTok{ }\NormalTok{sat\_mod }\OperatorTok{\%\textgreater{}\%}
\StringTok{      }\KeywordTok{update}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ states\_voting\_cleaned)}
...
Use the \texttt{states} data set. Fit a model predicting energy consumed per capita (\texttt{energy}) from the percentage of residents living in metropolitan areas (\texttt{metro}). Be sure to:
...
\tightlist
\item
  Plot the model using \texttt{plot()} to look for deviations from modeling assumptions.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  states\_en\_met \textless{}{-}}\StringTok{ }\NormalTok{states }\OperatorTok{\%\textgreater{}\%}\StringTok{ }
\StringTok{      }\KeywordTok{select}\NormalTok{(energy, metro)}
...
\includegraphics{R/Rmodels/figures/unnamed-chunk-106-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  mod\_en\_met \textless{}{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(energy }\OperatorTok{\textasciitilde{}}\StringTok{ }\NormalTok{metro, }\DataTypeTok{data =}\NormalTok{ states\_en\_met)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  states\_en\_met\_pop\_wst \textless{}{-}}\StringTok{ }\NormalTok{states }\OperatorTok{\%\textgreater{}\%}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{  mod\_en\_met\_pop\_waste \textless{}{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(energy }\OperatorTok{\textasciitilde{}}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{metro }\OperatorTok{+}\StringTok{ }\NormalTok{pop }\OperatorTok{+}\StringTok{ }\NormalTok{waste, }\DataTypeTok{data =}\NormalTok{ states\_en\_met\_pop\_wst)}
...
\begin{Shaded}
\begin{Highlighting}[]
  \CommentTok{\# add the interaction to a model}
\NormalTok{  sat\_expense\_by\_income \textless{}{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(csat }\OperatorTok{\textasciitilde{}}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{expense }\OperatorTok{+}\StringTok{ }\NormalTok{income }\OperatorTok{+}\StringTok{ }\NormalTok{expense }\OperatorTok{:}\StringTok{ }\NormalTok{income, }\DataTypeTok{data =}\NormalTok{ states)}

  \CommentTok{\# same as above, but shorter syntax}
\NormalTok{  sat\_expense\_by\_income \textless{}{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(csat }\OperatorTok{\textasciitilde{}}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{expense }\OperatorTok{*}\StringTok{ }\NormalTok{income, }\DataTypeTok{data =}\NormalTok{ states) }

  \CommentTok{\# show the regression coefficients table}
  \KeywordTok{summary}\NormalTok{(sat\_expense\_by\_income) }\OperatorTok{\%\textgreater{}\%}\StringTok{ }\KeywordTok{coef}\NormalTok{() }
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
  ...
  \KeywordTok{str}\NormalTok{(states}\OperatorTok{$}\NormalTok{region)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
  \CommentTok{\# convert \textasciigrave{}region\textasciigrave{} to a factor}
\NormalTok{  states \textless{}{-}}\StringTok{ }\NormalTok{states }\OperatorTok{\%\textgreater{}\%}
\StringTok{      }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{region =} \KeywordTok{factor}\NormalTok{(region))}
  ...
  \CommentTok{\# examine factor levels}
  \KeywordTok{levels}\NormalTok{(states}\OperatorTok{$}\NormalTok{region)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  sat\_region \textless{}{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(csat }\OperatorTok{\textasciitilde{}}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{region, }\DataTypeTok{data =}\NormalTok{ states) }
...
\begin{Highlighting}[]
  ...
  \KeywordTok{contrasts}\NormalTok{(states}\OperatorTok{$}\NormalTok{region)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  states \textless{}{-}}\StringTok{ }\NormalTok{states }\OperatorTok{\%\textgreater{}\%}
\StringTok{      }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{region =} \KeywordTok{relevel}\NormalTok{(region, }\DataTypeTok{ref =} \StringTok{"Midwest"}\NormalTok{))}
...
\NormalTok{  mod\_region \textless{}{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(csat }\OperatorTok{\textasciitilde{}}\StringTok{ }\DecValTok{1} \OperatorTok{+}\StringTok{ }\NormalTok{region, }\DataTypeTok{data =}\NormalTok{ states)}
...
\begin{Highlighting}[]
\NormalTok{  NH11 \textless{}{-}}\StringTok{ }\NormalTok{NH11 }\OperatorTok{\%\textgreater{}\%}
\StringTok{      }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{everwrk =} \KeywordTok{factor}\NormalTok{(everwrk, }\DataTypeTok{levels =} \KeywordTok{c}\NormalTok{(}\StringTok{"2 No"}\NormalTok{, }\StringTok{"1 Yes"}\NormalTok{)),}
...
\CommentTok{\# here we store the \textquotesingle{}base plot\textquotesingle{} {-} without geometric objects {-} in the object \textquotesingle{}p1\textquotesingle{}}
...
\CommentTok{\# we can then add geometric objects to our base plot \textquotesingle{}p1\textquotesingle{}}
...
\includegraphics{R/Rgraphics/figures/unnamed-chunk-227-1.pdf}
...
The UK \href{https://www.ons.gov.uk}{Office for National Statistics} provides yearly data on the most popular boys names going back to 1996. The data is provided separately for boys and girls and is stored in Excel spreadsheets.
...
Base R provides some string manipulation capabilities (see \texttt{?regex}, \texttt{?sub} and \texttt{?grep}), but we will use the \texttt{stringr} package within \texttt{tidyverse} because it is more user-friendly. \texttt{stringr} provides functions to:
...
strings. Here we want to detect the pattern ``Table 1'', and only return elements with this pattern. We can do that using the \texttt{str\_subset()} function:
...
In R, the most foundational data structure is the \textbf{vector}. Vectors are \emph{containers} that can hold a \emph{collection} of values. Vectors come in two basic forms:
...
\includegraphics{R/RDataWrangling/figures/unnamed-chunk-284-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{alice\_words[}\DecValTok{10}\NormalTok{:}\DecValTok{19}\NormalTok{] }\CommentTok{\# returns a list with words 11 through 19}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Split text on newlines to produce a list with one element per line. Store the result as \texttt{alice\_characters}. HINT: you can split on newlines using the \texttt{splitlines()} method.
\end{enumerate}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{alice\_characters }\OperatorTok{=}\NormalTok{ characters\_txt.splitlines()}
...
\tightlist
\item
  Test whether the length of the 3rd and 8th character's names are equal. Test whether the length of the 3rd character's name is greater than or equal to the length of the 6th character's name. Now test whether EITHER of the above conditions are true. HINT: use the \texttt{len()} function.
\end{enumerate}
...
\tightlist
\item
  Make sure you have both the text and the list of characters. Open and read both \texttt{Alice\_in\_wonderland.txt} and \texttt{Characters.txt} if you have not already done so.
\end{enumerate}
...
\tightlist
\item
  (BONUS, optional): Put the character counts computed above in a dictionary with character names as the keys and
...
\ControlFlowTok{for}\NormalTok{ character }\KeywordTok{in}\NormalTok{ characters\_txt.splitlines():}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{characters }\OperatorTok{=}\NormalTok{ characters\_txt.splitlines()}
\BuiltInTok{dict}\NormalTok{(}\BuiltInTok{zip}\NormalTok{(characters, num\_per\_character))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## {'Alice': 395, 'White Rabbit': 20, 'Mouse': 30, 'Dodo': 13, 'Lory': 7, 'Eaglet': 3, 'Duck': 3, 'Pat': 3, 'Bill the Lizard': 0, 'Puppy': 0, 'Caterpillar': 27, 'Duchess': 42, 'Cheshire Cat': 4, 'March Hare': 30, 'Hatter': 55, 'Dormouse': 40, 'Queen of Hearts': 3, 'Knave of Hearts': 3, 'King of Hearts': 0, 'Gryphon': 55, 'Mock Turtle': 53}
\end{verbatim}
...
If we are lucky we'll find a resource that returns the data we're looking for in a structured format like \href{https://json.org/}{JSON}.
...
\NormalTok{exhibit\_path }\OperatorTok{=} \StringTok{"search/load\_more"}
...
\NormalTok{    records\_per\_page }\OperatorTok{=}\NormalTok{ requests.get(exhibit\_url, }\OperatorTok{\textbackslash{}}
\NormalTok{        params }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}type\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}past{-}exhibition\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}page\textquotesingle{}}\NormalTok{: page\}).json()[}\StringTok{\textquotesingle{}records\textquotesingle{}}\NormalTok{]}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{first5Pages }\OperatorTok{=}\NormalTok{ [requests.get(exhibit\_url, }\OperatorTok{\textbackslash{}}
\NormalTok{    params }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}type\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}past{-}exhibition\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}page\textquotesingle{}}\NormalTok{: page\}).json()[}\StringTok{\textquotesingle{}records\textquotesingle{}}\NormalTok{] }\ControlFlowTok{for}\NormalTok{ page }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{6}\NormalTok{)]}
