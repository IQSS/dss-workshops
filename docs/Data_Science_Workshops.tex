...
\hypertarget{text-editors-integrated-development-environments-ides}{%
\subsection{Text editors \& Integrated Development Environments (IDEs)}\label{text-editors-integrated-development-environments-ides}}

There are different ways of interfacing with R. The two main ways are through \textbf{text editors} and \textbf{Integrated Development Environments (IDEs)}. Text editors and IDEs are not really separate categories; as you add features to a text editor it becomes more like an IDE. Some editors/IDEs are language-specific while others are general purpose --- typically provide language support via plugins. For these workshops we will use \href{https://rstudio.com/}{RStudio}; it is a good R-specific IDE with many useful features. Here are a few popular editors/IDEs that can be used with R:
...
\hypertarget{literate-programming-versus-source-code}{%
\subsection{Literate programming versus source code}\label{literate-programming-versus-source-code}}
...
\includegraphics{R/Rinstall/images/tidy_data.png}

We will use \texttt{tidyverse} packages throughout these R workshops,
...
\includegraphics{R/Rinstall/images/tidy_workflow.png}
...
\hypertarget{text-editors-integrated-development-environments-ides-1}{%
\subsubsection{Text editors \& Integrated Development Environments (IDEs)}\label{text-editors-integrated-development-environments-ides-1}}
...
\hypertarget{literate-programming-versus-source-code-1}{%
\subsubsection{Literate programming versus source code}\label{literate-programming-versus-source-code-1}}
...
\includegraphics{R/Rintro/figures/unnamed-chunk-30-1.pdf}
...
\hypertarget{creating-variables-within-groups}{%
\section{Creating variables within groups}\label{creating-variables-within-groups}}
...
\hypertarget{aggregating-variables-within-groups}{%
\section{Aggregating variables within groups}\label{aggregating-variables-within-groups}}

\textbf{GOAL: To learn how to aggregate data to create summaries.} In particular:
...
\includegraphics{R/Rintro/figures/unnamed-chunk-83-1.pdf}
...
\includegraphics{R/Rintro/figures/unnamed-chunk-89-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
  \CommentTok{## 1. Write a function that takes a file name as an argument and reads}
  \CommentTok{##    the worksheet containing "Table 1" from that file.}
...
\NormalTok{\}}
  ...
  \CommentTok{## 2. Test your function by using it to read *one* of the boys names}
  \CommentTok{##    Excel files.}
...
\KeywordTok{read_boys_names}\NormalTok{(boy_file_names[}\DecValTok{1}\NormalTok{], }\DataTypeTok{sheet_name =} \StringTok{"Table 1"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{glimpse}\NormalTok{()}
  ...
  \CommentTok{## 3. Use the `map` function to read data from all the Excel files,}
  \CommentTok{##    using the function you wrote in step 1.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{## 1.  Your task now is to use the `map()` function to apply each of these}
\CommentTok{##     transformations to all the elements in `boysNames`. }

\CommentTok{## }\AlertTok{NOTE}\CommentTok{: some Excel files include extra blank columns between the first and second }
\CommentTok{## set of `Name` and `Count` columns, resulting in different numeric suffixes}
\CommentTok{## for the second set of columns. You will need to use a regular expression}
\CommentTok{## to match each of these different column names. HINT: see the `?matches` function.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{## 1.  Turn the list of boys names `data.frames` into a single `data.frame`.}

\NormalTok{boysNames <-}\StringTok{ }\KeywordTok{bind_rows}\NormalTok{(boysNames)}
\KeywordTok{glimpse}\NormalTok{(boysNames)}


\CommentTok{## 2.  Create a new directory called `all` within `dataSets` and write the data to a `.csv` file. }
\CommentTok{##     HINT: see the `?dir.create` and `?write_csv` functions.}
...
\CommentTok{## 3.  What were the five most popular names in 2013?   }
...
\CommentTok{## How has the popularity of the name "ANDREW" changed over time?}
...
\hypertarget{software-materials-6}{%
...
\hypertarget{software-materials-7}{%
...
\hypertarget{software-materials-8}{%
