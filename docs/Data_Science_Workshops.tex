...
\includegraphics{R/Rmodels/figures/unnamed-chunk-128-1.pdf} \includegraphics{R/Rmodels/figures/unnamed-chunk-128-2.pdf} \includegraphics{R/Rmodels/figures/unnamed-chunk-128-3.pdf} \includegraphics{R/Rmodels/figures/unnamed-chunk-128-4.pdf}
...
1. Multiple Excel sheets in each file, each with a different name, but each file contains a \texttt{Table\ 1}.
...
4. The second worksheet \texttt{2015boysnamesfinal.xlsx} contains extra columns between the data of interest, resulting in the second set of columns (ranks 51-100) being placed in a different position.
...
1. Write a function that takes a data frame as an argument and returns a modified version, which keeps only columns that include the strings \texttt{Name} and \texttt{Count} in the column names. HINT: see the \texttt{?matches} function.
...
1. Create a new function called \texttt{cleanupNamesData} that:
