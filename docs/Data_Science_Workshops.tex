...
\KeywordTok{names}\NormalTok{(boysNames) }\CommentTok{# returns NULL}

\KeywordTok{names}\NormalTok{(boysNames) <-}\StringTok{ }\NormalTok{Years }\CommentTok{# assigns years to list names}

\KeywordTok{names}\NormalTok{(boysNames) }\CommentTok{# returns the years}

\KeywordTok{head}\NormalTok{(boysNames) }\OperatorTok{%>%}\StringTok{ }\KeywordTok{glimpse}\NormalTok{(boysNames) }
\end{Highlighting}
\end{Shaded}
...
\NormalTok{## 3.  Finally, repeat the previous exercise, this time working with the data in one big data.frame.}
