...
complex & 1+4i (complex numbers with real and imaginary
parts)\tabularnewline
\bottomrule
\end{longtable}
...
\tightlist
\item
  \textbf{atomic vectors} (aka \textbf{vectors}): these hold
  \textbf{homogeneous elements}
\item
  \textbf{generic vectors} (aka \textbf{lists}): these hold
  \textbf{heterogeneous elements}
\end{enumerate}
...
\textbf{coerce} the vector to the simplist type required to store all
the information. The order of coercion is: logical -\textgreater{}
integer -\textgreater{} numeric -\textgreater{} complex -\textgreater{}
character -\textgreater{} list.

You can query the \textbf{type} of an object using the \texttt{typeof()}
function. Note that \textbf{type} is not the same as \textbf{class}: the
latter is a property assigned to an object that determines how generic
functions operate with it. The class of an object can be queried using
