...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{import delimited }\KeywordTok{using} \StringTok{"http://tutorials.iq.harvard.edu/R/Rgraphics/dataSets/EconomistData.csv"}
\KeywordTok{sum}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \textbf{atomic}: only hold elements of the same type --- \textbf{homogeneous}
\item
  \textbf{lists}: can hold elements of different types --- \textbf{heterogeneous}
\end{enumerate}
...
\hypertarget{download-workshop-materials}{%
\subsubsection{Download workshop materials}\label{download-workshop-materials}}
...
\item
  Click \texttt{upload}, then in the pop-up window select the \emph{PythonIntro} folder on the desktop
\item
...
\texttt{PythonWebScrape.ipynb} file in the \texttt{PythonWebScrape} folder you
...
This workshop is relatively \emph{informal}, \emph{example-oriented}, and
\emph{hands-on}. We will learn by working through an example web scraping
project.

Note that this is \textbf{not} an introductory workshop. Familiarity with
...
\href{https://dss.iq.harvard.edu/workshop-materials\#widget-0}{IQSS Introduction to Python}.
...
\hypertarget{making-requests-using-python}{%
\subsection{Making requests using Python}\label{making-requests-using-python}}
