...
is. The table below lists functions from the \texttt{readr} package,
which is part of \texttt{tidyverse}, that can import data from common
plain-text formats.
...
will use the \texttt{dplyr} package to filter, select, and arrange data.
...
of a \texttt{data.frame}. For example, we can use it to compute the log
of the number of boys and girls given each name in each year:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{baby_names <-}\StringTok{ }\KeywordTok{mutate}\NormalTok{(baby_names, }\DataTypeTok{logCount =} \KeywordTok{log10}\NormalTok{(Count))}
...
We can also \texttt{select} a subset of columns from the baby names
data:
...
\StringTok{  }\KeywordTok{group_by}\NormalTok{(Year, Sex) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Proportion =}\NormalTok{ Count}\OperatorTok{/}\KeywordTok{sum}\NormalTok{(Count))}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\NormalTok{ungroup }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{group_by}\NormalTok{(Year, Sex) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Rank =} \KeywordTok{rank}\NormalTok{(}\OperatorTok{-}\NormalTok{Count))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{top1 <-}\StringTok{ }\KeywordTok{filter}\NormalTok{(baby_names, Rank }\OperatorTok{==}\StringTok{ }\DecValTok{1}\NormalTok{)}
...
\NormalTok{girls_and_boys <-}\StringTok{ }\NormalTok{girls_and_boys }\OperatorTok{%>%}\StringTok{ }\KeywordTok{ungroup}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  \tightlist
  \item
    \href{https://training.rcs.hbs.org/workshops}{Research Computing
    Services workshops}
  \item
    \href{https://training.rcs.hbs.org/workshop-materials}{other HBS RCS
    resources}
  \item
...
While R's built-in packages are powerful, in recent years there has been
a big surge in well-designed \emph{contributed packages} for R.\\
...
You can add a geom to a plot using the \texttt{+} operator. You can get
a list of available geometric objects using the code below:
...
associated with it, some of which are required and some optional:
...
\textbf{\texttt{x}},\texttt{y},\texttt{alpha},\texttt{color},\texttt{fill},\texttt{group},\texttt{shape},\texttt{size},\texttt{stroke}\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.17\columnwidth}\raggedright\strut
...
Line plot\strut
\end{minipage} & \begin{minipage}[t]{0.59\columnwidth}\raggedright\strut
\texttt{x},\texttt{y},\texttt{alpha},\texttt{color},\texttt{linetype},\texttt{size}\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.17\columnwidth}\raggedright\strut
...
Bar chart\strut
\end{minipage} & \begin{minipage}[t]{0.59\columnwidth}\raggedright\strut
\texttt{x},\texttt{y},\texttt{alpha},\texttt{color},\texttt{fill},\texttt{group},\texttt{linetype},\texttt{size}\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.17\columnwidth}\raggedright\strut
...
Boxplot\strut
\end{minipage} & \begin{minipage}[t]{0.59\columnwidth}\raggedright\strut
\texttt{x},\texttt{lower},\texttt{upper},\texttt{middle},\texttt{ymin},\texttt{ymax},\texttt{alpha},\texttt{color},\texttt{fill}\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.17\columnwidth}\raggedright\strut
...
Density plot\strut
\end{minipage} & \begin{minipage}[t]{0.59\columnwidth}\raggedright\strut
\texttt{x},\texttt{y},\texttt{alpha},\texttt{color},\texttt{fill},\texttt{group},\texttt{linetype},\texttt{size},\texttt{weight}\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.17\columnwidth}\raggedright\strut
\texttt{geom\_smooth()}\strut
\end{minipage} & \begin{minipage}[t]{0.16\columnwidth}\raggedright\strut
Smoothed means\strut
\end{minipage} & \begin{minipage}[t]{0.59\columnwidth}\raggedright\strut
\texttt{x},\texttt{y},\texttt{alpha},\texttt{color},\texttt{fill},\texttt{group},\texttt{linetype},\texttt{size},\texttt{weight}\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.17\columnwidth}\raggedright\strut
...
\end{minipage} & \begin{minipage}[t]{0.16\columnwidth}\raggedright\strut
Jittered points\strut
\end{minipage} & \begin{minipage}[t]{0.59\columnwidth}\raggedright\strut
\texttt{x},\texttt{y},\texttt{alpha},\texttt{color},\texttt{fill},\texttt{group},\texttt{shape},\texttt{size},\texttt{stroke}\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
