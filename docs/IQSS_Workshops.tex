...
\section{Ex 1: Reading the baby names
data}\label{ex-1-reading-the-baby-names-data}
...
\section{Ex 2: Peak popularity of your
name}\label{ex-2-peak-popularity-of-your-name}
...
\section{Ex 3: Plotting peak popularity of your
name}\label{ex-3-plotting-peak-popularity-of-your-name}
...
\section{Ex 4: Most popular names}\label{ex-4-most-popular-names}
...
\section{Ex 5: Popularity of most popular
names}\label{ex-5-popularity-of-most-popular-names}
...
\subsection{Ex 4: solution}\label{ex-4-solution}
...
\subsection{Ex 5: solution}\label{ex-5-solution}
...
\subsection{Inspect and load data}\label{inspect-and-load-data}
...
We're going to use the \texttt{states} data first. The \emph{states.dta}
data comes from
...
\section{Ex 0: ordinary least squares
regression}\label{ex-0-ordinary-least-squares-regression}
...
\NormalTok{  states.data}\OperatorTok{$}\NormalTok{region <-}\StringTok{ }\KeywordTok{relevel}\NormalTok{(states.data}\OperatorTok{$}\NormalTok{region, }\DataTypeTok{ref =} \StringTok{"NorthEast"}\NormalTok{)}
...
\section{Ex 1: interactions and
factors}\label{ex-1-interactions-and-factors}
...
\NormalTok{  eff2 <-}\StringTok{ }\KeywordTok{allEffects}\NormalTok{(hyp.out, }\DataTypeTok{xlevels =} \KeywordTok{list}\NormalTok{(age_b, }\KeywordTok{seq}\NormalTok{(}\DecValTok{20}\NormalTok{, }\DecValTok{80}\NormalTok{, }\DataTypeTok{by =} \DecValTok{5}\NormalTok{)))}
...
\section{Ex 2: logistic regression}\label{ex-2-logistic-regression}
...
\section{Ex 3: multilevel modeling}\label{ex-3-multilevel-modeling}
...
(HRS), and group (GRP). Each of these variables has two variants - a
group mean version that replicates each group mean for every individual,
and a within-group version where the group mean is subtracted from each
individual response. The group mean version is designated with a G.
(e.g., G.HRS), and the within-group version is designated with a W.
(e.g., W.HRS).
\end{quote}
...
\section{Ex 3: cleanup all the data}\label{ex-3-cleanup-all-the-data}
...
\section{Ex 4: make one big table}\label{ex-4-make-one-big-table}
