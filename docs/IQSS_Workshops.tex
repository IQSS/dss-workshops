...
For example, the default statistic for \texttt{geom\_histogram} is
\texttt{stat\_bin}:
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{ggplot}\NormalTok{(housing_sum, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x=}\NormalTok{State, }\DataTypeTok{y=}\NormalTok{Home_Value)) }\OperatorTok{+}\StringTok{ }
