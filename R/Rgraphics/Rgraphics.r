...
# ### Setting arguments
#
# Each `geom_` function has a default statistic that it is paired with and, likewise, each `stat_` function has a default geometic object that it is paired with. For example, the default statistic for `geom_histogram()` is `stat_bin()`, while the default geometric object for `stat_bin()` is `geom_bar()`. These defaults can be changed. 
#
# Arguments to `stat_` functions can be passed through `geom_` functions and vice versa. This can be slightly annoying, because in order to specify some arguments you have to first determine which statistic the geometric object uses, then determine the arguments to that statistic.
#
# Let's look at the arguments for `geom_histogram()`:

args(geom_histogram)

# Notice that the third argument is `stat = "bin"`, which calls the `stat_bin()` function. This is how the histogram geom is, by default, paired with the bin statistic. The fifth argument is `...`, which allows us to pass arguments to `stat_bin()` through our call to `geom_histogram()`. Now let's look at the arguments to `stat_bin()`:

args(stat_bin)

# We can see that there is a `binwidth` argument to `stat_bin()` that is not present for `geom_histogram()`. But, we can still use this `binwidth` argument in `geom_histogram()` --- it will automatically be passed to its paired statistic function. For example, here is the default histogram of `Home_Value`:

p2 <- ggplot(housing, aes(x = Home_Value))
p2 + geom_histogram()

# We can change the binning scheme by passing the `binwidth` argument through `geom_histogram()` to the `stat_bin()` function:

p2 + geom_histogram(stat = "bin", binwidth = 4000)

# Here is a list of geometric objects and their default statistics <https://ggplot2.tidyverse.org/reference/>
