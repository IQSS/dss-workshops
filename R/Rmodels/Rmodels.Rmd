...
Install the [tidyverse](https://www.tidyverse.org/) suite of packages and the `readr` package:
...
```{r}
  ...
  states.data <- read_rds("dataSets/states.rds") 
  #get labels
...
```{r}
  ...
  sat.expense.by.percent <- lm(csat ~ expense + income + expense : income, data=states.data)
...
```{r}
  ...
  states.data$region <- relevel(states.data$region, ref = "Midwest")
...
```{r}
  ...
  eff2 <- allEffects(hyp.out, xlevels = list(age_p, seq(20, 80, by = 5)))
  plot(eff)
...
But, we can calculate approximate p-values using the `lmerTest` package.
...
> Variables are Leadership Climate (LEAD), Well-Being (WBEING), Work Hours (HRS), and cluster group (GRP). 
