...
particular, we will use the `dplyr` package to filter,
...
of a `data.frame`. For example, we can use it to compute the 
log of the number of boys and girls given each name in each year:

```{r}
baby_names <- mutate(baby_names, logCount = log10(Count))
...
We can also `select` a subset of columns from the baby names data:
...
girls_and_boys <- girls_and_boys %>% ungroup()
```
