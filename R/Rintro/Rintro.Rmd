...
* [Download workshop materials](http://tutorials.iq.harvard.edu/R/Rintro.zip)
...
```{r}
  ...
  # setwd("~/Desktop/Rintro") # UNIX-based
  # setwd("C:/Users/Desktop/Rintro") # MS Windows
```
...
Make sure you have installed the `readr` package and attached it with `library(readr)`. Baby names data are available at `"http://tutorials.iq.harvard.edu/data/babyNames.csv"`.
...
is part of a powerful collection of R packages called `tidyverse`, which are 
designed specifically for data science . All packages included in `tidyverse`
...
**Popularity of most popular names**
