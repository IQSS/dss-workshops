...
2.  In the lower left-hand side window (called the `console`), at the command prompt `>` type the following and press enter:
...
2.  there is no package `rlang`...
    * Solution: run this command in the console `install.packages("dplyr")`. If a choice appears that says something  like "Do you want to install from sources the package which needs compilation?"", type “No” in the console.
...
**If you still have not successfully installed `tidyverse` after troubleshooting, please stop by the training room 20 minutes before the start of your workshop so we can help you.**


## Installing `rmarkdown` (optional)

1.  We can also install the `rmarkdown` package, which will allow us to
...
2.  We can check that `rmarkdown` has installed correctly by connecting it to our R session. 
    Type the following in the console and press enter:

    `library(rmarkdown)`

    If successful, you should see a command prompt `>` and no messages displayed in the console. 
    **If you see error or warning messages, please stop by the training room 20 minutes before the start of your workshop.**
