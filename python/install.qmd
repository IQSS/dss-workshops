...
1. Start Positron and go to `File -> Open Folder`; choose the workshop materials folder you unzipped (for the introduction, `intro`).
2. Open a terminal inside Positron (`Terminal -> New Terminal`) and run:
   ...
   This installs Python {{< var python.version >}} if you do not have it, and the packages listed in the folder's `pyproject.toml`, into a `.venv` folder inside the materials folder. It takes a minute or two the first time.
