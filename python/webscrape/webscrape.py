...
#
# If you need an introduction to Python or a refresher, we recommend our [Python Introduction](https://iqss.github.io/dss-workshops/python/intro.html).
#
# ### Goals
#
...
#
# 1. Start Positron.
# 2. Go to `File -> Open Folder` and choose the `python-webscrape` folder from the workshop materials on your desktop. If Positron asks which Python to use, choose the one in `.venv` (the [installation page](https://iqss.github.io/dss-workshops/python/install.html) explains `uv sync`, which creates it).
# 3. In the Explorer on the left, click the file with the word "BLANK" in the name (`webscrape_BLANK.ipynb`). It opens in Positron's notebook editor, with the kernel picker at the top right showing the Python from `.venv`.
#
...
#
# ![](images/dev_tools_right_click.png)
#
# ![](images/dev_tools_inspect.png)
#
...
#
# ![](images/dev_tools_figcaption.png)
#
...
#
# It is sometimes necessary (or sometimes just easier) to use a web browser as an intermediary rather than communicate directly with a web service. This method of using a "browser driver" has the advantage of being able to use the javascript engine and session management features  of a web browser; the main disadvantage is that it is slower and tends to be more fragile than using `requests` or `scrapy` to make requests directly from Python. For small scraping projects involving complicated sites with CAPTHAs or lots of complicated javascript using a browser driver can be a good option. More information is available at <https://www.selenium.dev/documentation/webdriver/>.
#
# ## Wrap-up
#
# ### Feedback
