---
title: "Python Introduction (BLANK)"
jupyter: python3
format: html
execute:
  warning: false
...
```{python}
# the [:500] gets the first 500 characters -- more on this later.
```
...
# returns first element - the number 1 (yes, the index counts from zero!)
# returns second element - the letter "b"
# returns a list with only the first 3 elements, but index is of length 4 (0 to 3) because last index is excluded
# returns a list with elements 3, "D", 5
# returns last element - the number 6 
# returns a list with last 4 elements
```

```{python}
# returns a list with words 11 through 19
# returns a list with the last 10 words
```
...
```{python}
# counts elements in a data structure
```
...
```{python}
# counts unique elements in a data structure
```
...
```{python}
# explicit printing --- formats text nicely

# returns content without printing it
```
